...
public class RecursionQ2 {
    public static void main(String[] args) {
        ...
        permWithSpaces("abc", "");
        permWithCaseChange("abc", "");
        permWithLetterCase("a1b2", new StringBuilder());
        permWithLetterCase("C", new StringBuilder());
    }

    private static void permWithLetterCase(String input, StringBuilder output) {
        if (input.length() == 0) {
        ...
        if (c >= 'a' && c <= 'z')
            permWithLetterCase(input.substring(1), new StringBuilder(output) .append ((char) (c + 'A' - 'a')));
        else if (c >= 'A' && c <= 'Z')
            permWithLetterCase(input.substring(1), new StringBuilder(output) .append ((char) (c - 'A' + 'a')));
        permWithLetterCase(input.substring(1), new StringBuilder(output) .append(c) );
    }

    private static void permWithCaseChange(String input, String output) {
        if (input.length() == 0) {
            System.out.println(output);
            return;
        }
        permWithCaseChange(input.substring(1), output + input.charAt(0));
        permWithCaseChange(input.substring(1), output + (char) (input.charAt(0) + 'A' - 'a'));
    }

    private static void permWithSpaces(String input, String output) {
        if (input.length() == 0) {
            System.out.println(output);
            return;
        }

        permWithSpaces(input.substring(1), output + input.charAt(0));
        if (output.length() != 0) permWithSpaces(input.substring(1), output + "_" + input.charAt(0));
